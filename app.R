...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    hr(),
    selectInput("species", "Seleccionar Especie:",
                choices = c("Litopenaeus occidentalis" = "L.occidentalis", "Xiphopenaeus riveti" = "X.rivetti",
                            "Solenocera agassizii" = "S.agassizii", "Penaeus brevirostris" = "P.brevirostris",
                            "Penaeus californiensis" = "P.californiensis"), selected = "L.occidentalis"),
    hr(),
    selectInput("month", "Seleccionar Mes:",
                choices = c("Enero" = "jan", "Febrero" = "feb", "Marzo" = "mar", "Abril" = "apr", "Mayo" = "may", "Junio" = "jun",
                            "Julio" = "jul", "Agosto" = "aug", "Septiembre" = "sep", "Octubre" = "oct", "Noviembre" = "nov", "Diciembre" = "dec"),
                selected = "jan"),
    hr(),
    radioButtons("scenario", "Escenario Futuro 2050:", choices = c("Optimista (SSP 1-2.6)" = "26", "Pesimista (SSP 5-8.5)" = "85"), selected = "26"),
    hr(),
    sliderInput("threshold", "Umbral de presencia (%):", min = 10, max = 70, value = 50, step = 5),
    hr(),
    sliderInput("opacity", "Opacidad del raster:", min = 0.3, max = 1, value = 0.8, step = 0.1),
    pickerInput("palette", "Paleta de colores:", choices = c("viridis", "plasma", "inferno", "magma", "turbo"), selected = "viridis")
  ),
  dashboardBody(
    tags$head(tags$style(HTML(".content-wrapper { background-color: #ecf0f5; } .box { border-top: 3px solid #3c8dbc; } .leaflet-container { background: #ffffff; }"))),
    tabItems(
      tabItem(tabName = "maps",
              fluidRow(infoBoxOutput("area_actual", width = 3), infoBoxOutput("area_futura", width = 3), infoBoxOutput("area_perdida", width = 3), infoBoxOutput("area_ganada", width = 3)),
              fluidRow(
                box(title = textOutput("title_present"), status = "primary", solidHeader = TRUE, width = 4, height = 600, leafletOutput("map_present", height = 520)),
                box(title = textOutput("title_future"), status = "warning", solidHeader = TRUE, width = 4, height = 600, leafletOutput("map_future", height = 520)),
                box(title = "Cambios en Idoneidad Ambiental", status = "info", solidHeader = TRUE, width = 4, height = 600, leafletOutput("map_changes", height = 520))
              ),
              fluidRow(box(title = "Análisis Cuantitativo de Cambios", status = "info", solidHeader = TRUE, width = 12, plotOutput("changes_barplot", height = 300)))
      ),
      tabItem(tabName = "uacs",
              fluidRow(box(title = "Mapa con UACs", status = "primary", solidHeader = TRUE, width = 12, height = 600, leafletOutput("map_uacs", height = 520))),
              fluidRow(box(title = "Análisis Detallado por UAC", status = "info", solidHeader = TRUE, width = 12, DTOutput("table_uac_complete"))),
              fluidRow(box(title = "Comparación Visual por UAC", status = "primary", solidHeader = TRUE, width = 12, plotOutput("uac_comparison_plot", height = 400)))
      ),
      tabItem(tabName = "temporal",
              fluidRow(box(title = "Variación Mensual del Área de Distribución", status = "primary", solidHeader = TRUE, width = 12, plotOutput("spider_plot", height = 650))),
              fluidRow(box(title = "Resumen de Resultados por Especie", status = "success", solidHeader = TRUE, width = 12, verbatimTextOutput("species_summary"))),
              fluidRow(box(title = "Tabla de Datos Mensuales", status = "info", solidHeader = TRUE, width = 12, DTOutput("spider_data_table")))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  raster_data <- reactiveValues(present_raw = NULL, future_raw = NULL, present_prob = NULL, future_prob = NULL,
                                present_binary = NULL, future_binary = NULL, changes = NULL, uacs = NULL, loaded = FALSE, name_column = NULL)
  detect_name_column <- function(sf_object) {
    possible_cols <- c("nombre", "NOMBRE", "Nombre", "name", "NAME", "Name", "UAC", "uac", "id", "ID", "region", "REGION")
  ...
  calculate_monthly_areas <- function(species, base_path, threshold) {
    ...
    resultados <- data.frame(Mes = meses_nombres, Presente = numeric(12), Futuro_2050_SSP126 = numeric(12),
                            Futuro_2050_SSP585 = numeric(12), Presente_km2 = numeric(12),
                            Futuro_2050_SSP126_km2 = numeric(12), Futuro_2050_SSP585_km2 = numeric(12), stringsAsFactors = FALSE)
  ...
  generate_species_summary <- function(datos, species_name) {
    ...
    if (cambio_ssp126_km2 > 0) {
      texto_ssp126 <- paste0("una expansión de ", format(round(cambio_ssp126_km2, 2), big.mark = ","), " km² (+", round(abs(cambio_ssp126_porc), 2), "% de ganancia)")
    } else {
      texto_ssp126 <- paste0("una contracción de ", format(round(abs(cambio_ssp126_km2), 2), big.mark = ","), " km² (", round(cambio_ssp126_porc, 2), "% de pérdida)")
    }
    if (cambio_ssp585_km2 > 0) {
      texto_ssp585 <- paste0("una expansión significativa a ", format(round(prom_ssp585_km2, 2), big.mark = ","), " km² (", round(prom_ssp585_porc, 2), "%), equivalente a una ganancia de ", format(round(cambio_ssp585_km2, 2), big.mark = ","), " km² (+", round(abs(cambio_ssp585_porc), 2), "% respecto al presente)")
    } else {
      texto_ssp585 <- paste0("una reducción drástica a ", format(round(prom_ssp585_km2, 2), big.mark = ","), " km² (", round(prom_ssp585_porc, 2), "%), equivalente a una pérdida de ", format(round(abs(cambio_ssp585_km2), 2), big.mark = ","), " km² (", round(cambio_ssp585_porc, 2), "% respecto al presente)")
    }
    parrafo <- paste0("El análisis del área idónea para la distribución potencial de ", species_name, " reveló un área promedio de ", format(round(prom_presente_km2, 2), big.mark = ","), " km² (", round(prom_presente_porc, 2), "% de las Unidades Ambientales Costeras, ", format(area_uac, big.mark = ","), " km²) en el período presente. ", "Las proyecciones bajo escenarios climáticos contrastantes muestran tendencias divergentes para 2050: ", "bajo el escenario optimista SSP1-2.6, se proyecta un área de ", format(round(prom_ssp126_km2, 2), big.mark = ","), " km² (", round(prom_ssp126_porc, 2), "%), representando ", texto_ssp126, ". ", "En contraste, el escenario pesimista SSP5-8.5 proyecta ", texto_ssp585, ".")
    return(parrafo)
  }
    ...
    tryCatch({
      withProgress(message = 'Cargando datos...', value = 0, {
        ...
        if (!file.exists(uac_path)) {
          ...
          showNotification("Shapefile UACs no encontrado. Reinicie la aplicación.", type = "error", duration = NULL)
          return(FALSE)
        }
        uacs <- tryCatch({
          ...
          message("Validando geometrías...")
          if (!all(st_is_valid(shp))) {
            message("Geometrías inválidas detectadas. Reparando...")
            shp <- st_make_valid(shp)
            message("Geometrías reparadas exitosamente")
          } else {
            message("Todas las geometrías son válidas")
          }
          message("Simplificando geometrías para optimizar rendimiento...")
          shp <- st_simplify(shp, preserveTopology = TRUE, dTolerance = 0.001)
          message("Geometrías simplificadas")
          shp
        }, error = function(e) {
          message("ERROR al leer shapefile: ", e$message)
          showNotification(paste("Error crítico al cargar shapefile:", e$message), type = "error", duration = NULL)
          return(NULL)
        })
        if (is.null(uacs)) {
          return(FALSE)
        ...
        if (nrow(uacs) == 0) {
          ...
          showNotification("El shapefile UACs no contiene datos válidos.", type = "error", duration = NULL)
          return(FALSE)
        }
        name_col <- detect_name_column(uacs)
        if (is.null(name_col)) {
          message("ERROR: No se pudo detectar columna de nombres")
          showNotification("No se pudo identificar la columna de nombres en el shapefile.", type = "error", duration = NULL)
          return(FALSE)
        }
        raster_data$name_column <- name_col
        message("Usando columna: '", name_col, "' para nombres de UACs")
        uacs <- st_transform(uacs, crs = 4326)
        message("Limpiando encoding de caracteres...")
        uacs_df <- st_drop_geometry(uacs)
        for (col in names(uacs_df)) {
          if (is.character(uacs_df[[col]])) {
            uacs[[col]] <- iconv(uacs_df[[col]], from = "", to = "UTF-8", sub = "")
          }
        }
        ...
        pres_bin_vals <- ifelse(is.na(pres_vals), NA, ifelse(pres_vals >= input$threshold, 1, 0))
        fut_bin_vals <- ifelse(is.na(fut_vals), NA, ifelse(fut_vals >= input$threshold, 1, 0))
      ...
      showNotification(paste("Error al cargar datos:", e$message), type = "error", duration = NULL)
  ...
  month_names <- c("jan"="Enero", "feb"="Febrero", "mar"="Marzo", "apr"="Abril", "may"="Mayo", "jun"="Junio",
  ...
  output$title_future <- renderText({
    paste("Proyección 2050 (SSP", gsub("26", "1-2.6", gsub("85", "5-8.5", input$scenario)), ") -", species_names[input$species], "-", month_names[input$month])
  })
  output$map_present <- renderLeaflet({
    req(raster_data$present_prob)
    pal <- colorNumeric(palette = input$palette, domain = c(0, 100), na.color = "transparent")
    m <- leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
      ...
      addLegend(pal = pal, values = c(0, 25, 50, 75, 100), title = "Probabilidad (%)", position = "bottomright", opacity = 1) %>%
      addScaleBar(position = "bottomleft")
    m %>% onRender("function(el, x) { var map = this; map.on('moveend', function() { Shiny.setInputValue('sync_present', { center: map.getCenter(), zoom: map.getZoom(), timestamp: Date.now() }); }); }")
  })
  output$map_future <- renderLeaflet({
    ...
    m <- leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
      ...
      addLegend(pal = pal, values = c(0, 25, 50, 75, 100), title = "Probabilidad (%)", position = "bottomright", opacity = 1) %>%
      addScaleBar(position = "bottomleft")
    m %>% onRender("function(el, x) { var map = this; map.on('moveend', function() { Shiny.setInputValue('sync_future', { center: map.getCenter(), zoom: map.getZoom(), timestamp: Date.now() }); }); }")
  })
  output$map_changes <- renderLeaflet({
    req(raster_data$changes)
    pal <- colorFactor(palette = c("#CCCCCC", "#E74C3C", "#F39C12", "#27AE60"), domain = 0:3, na.color = "transparent")
    m <- leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
      addRasterImage(raster_data$changes, colors = pal, opacity = 1, project = FALSE, method = "ngb") %>%
      addLegend(colors = c("#E74C3C", "#F39C12", "#27AE60", "#CCCCCC"), labels = c("Cambios Negativos", "Sin Cambios", "Cambios Positivos", "Ausencia"), title = "Categorías", position = "bottomright") %>%
      addScaleBar(position = "bottomleft")
    m %>% onRender("function(el, x) { var map = this; map.on('moveend', function() { Shiny.setInputValue('sync_changes', { center: map.getCenter(), zoom: map.getZoom(), timestamp: Date.now() }); }); }")
  })
  output$map_uacs <- renderLeaflet({
    req(raster_data$uacs, raster_data$name_column)
    tryCatch({
      message("=== INICIANDO RENDER DE MAPA UACs ===")
      pal <- colorFactor(palette = c("#CCCCCC", "#E74C3C", "#F39C12", "#27AE60"), domain = 0:3, na.color = "transparent")
      uacs <- raster_data$uacs
      name_col <- raster_data$name_column
      message("Número de UACs: ", nrow(uacs))
      message("Columna de nombres: ", name_col)
      message("Clase del objeto: ", class(uacs)[1])
      uac_names <- tryCatch({
        as.character(st_drop_geometry(uacs)[[name_col]])
      }, error = function(e) {
        message("Error extrayendo nombres, usando índices")
        paste0("UAC_", 1:nrow(uacs))
      })
      uac_names <- iconv(uac_names, from = "", to = "ASCII//TRANSLIT")
      uac_names[is.na(uac_names) | uac_names == ""] <- paste0("UAC_", which(is.na(uac_names) | uac_names == ""))
      message("Nombres procesados: ", paste(uac_names, collapse = ", "))
      m <- leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap)
      if (!is.null(raster_data$changes)) {
        message("Agregando capa de cambios...")
        m <- tryCatch({
          m %>% addRasterImage(raster_data$changes, colors = pal, opacity = 0.7, project = FALSE, method = "ngb")
        }, error = function(e) {
          message("No se pudo agregar raster: ", e$message)
          m
        })
      }
      message("Agregando polígonos...")
      m <- m %>% addPolygons(data = uacs, color = "#000000", weight = 2, fillOpacity = 0, label = uac_names, popup = paste0("<strong>", uac_names, "</strong>"),
                            highlightOptions = highlightOptions(weight = 4, color = "#FFFF00", bringToFront = TRUE)) %>%
        addLegend(colors = c("#E74C3C", "#F39C12", "#27AE60", "#CCCCCC"), labels = c("Cambios Negativos", "Sin Cambios", "Cambios Positivos", "Ausencia"), title = "Categorías", position = "bottomright") %>%
        addScaleBar(position = "bottomleft")
      message("=== MAPA UACs RENDERIZADO EXITOSAMENTE ===")
      return(m)
    }, error = function(e) {
      message("=== ERROR CRÍTICO EN RENDER MAPA UACs ===")
      message("Error: ", e$message)
      message("Clase: ", class(e)[1])
      message("Traceback completo:")
      print(traceback())
      showNotification(paste("Error al renderizar mapa UACs:", e$message), type = "error", duration = NULL)
      return(leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
               addControl(html = paste0("<div style='background: rgba(255,255,255,0.9); padding: 15px; border-radius: 5px; border: 2px solid #E74C3C;'>",
                                       "<strong style='color: #E74C3C;'>Error al cargar mapa de UACs</strong><br>",
                                       "Consulte la tabla y gráficos más abajo para el análisis detallado.</div>"), position = "topright"))
    })
  })
  ...
  observeEvent(input$sync_present, {
    ...
    if (current_time - last_sync_time() > 0.5 && last_sync_source() != "present") {
      ...
      leafletProxy("map_future") %>% setView(lng = v$center$lng, lat = v$center$lat, zoom = v$zoom)
      leafletProxy("map_changes") %>% setView(lng = v$center$lng, lat = v$center$lat, zoom = v$zoom)
    }
  }, ignoreInit = TRUE)
  observeEvent(input$sync_future, {
    ...
    if (current_time - last_sync_time() > 0.5 && last_sync_source() != "future") {
      ...
      v <- input$sync_future
      leafletProxy("map_present") %>% setView(lng = v$center$lng, lat = v$center$lat, zoom = v$zoom)
      leafletProxy("map_changes") %>% setView(lng = v$center$lng, lat = v$center$lat, zoom = v$zoom)
    }
  }, ignoreInit = TRUE)
  ...
  calculate_areas <- reactive({
    ...
    tryCatch({
      ...
      list(negativos = round(sum(vals == 1, na.rm = TRUE) * pixel_area, 2), sin_cambios = round(sum(vals == 2, na.rm = TRUE) * pixel_area, 2),
           positivos = round(sum(vals == 3, na.rm = TRUE) * pixel_area, 2), area_actual = round((sum(vals == 1, na.rm = TRUE) + sum(vals == 2, na.rm = TRUE)) * pixel_area, 2),
  ...
  calculate_uac_areas <- reactive({
    req(raster_data$changes, raster_data$uacs, raster_data$present_binary, raster_data$future_binary, raster_data$name_column)
    tryCatch({
      uacs <- raster_data$uacs
      ...
      for (i in 1:nrow(uacs)) {
        ...
        results[[uac_name]] <- list(UAC = uac_name, Area_Presente_km2 = round(area_presente, 2), Area_Futura_km2 = round(area_futura, 2),
                                    Cambio_Total_km2 = round(area_futura - area_presente, 2),
                                    Cambio_Porcentaje = round(ifelse(area_presente > 0, ((area_futura - area_presente) / area_presente) * 100, 0), 2),
                                    Perdida_km2 = round(negativos, 2), Sin_Cambios_km2 = round(sin_cambios, 2), Ganancia_km2 = round(positivos, 2))
      }
      df_result <- do.call(rbind, lapply(results, as.data.frame))
      tryCatch({
        orden_uacs <- c("Norte_Choco", "Baudo_San Juan", "Malaga_Buenaventura", "Llanura_Aluvial_Sur", "Norte_Chocó", "Baudó_San Juan", "Málaga_Buenaventura")
        df_result$UAC <- factor(df_result$UAC, levels = intersect(orden_uacs, df_result$UAC))
      ...
      showNotification(paste("Error al calcular áreas por UAC:", e$message), type = "error", duration = 10)
      return(NULL)
    })
  })
  output$area_actual <- renderInfoBox({
    areas <- calculate_areas()
    infoBox("Área Potencial Actual", paste(format(areas$area_actual, big.mark=","), "km²"), icon = icon("map-marked"), color = "blue", fill = TRUE)
  })
  output$area_futura <- renderInfoBox({
    areas <- calculate_areas()
    infoBox("Área Potencial Futura", paste(format(areas$area_futura, big.mark=","), "km²"), icon = icon("map-marked-alt"), color = "yellow", fill = TRUE)
  })
  output$area_perdida <- renderInfoBox({
    areas <- calculate_areas()
    infoBox("Área con Pérdida", paste(format(areas$negativos, big.mark=","), "km²"), icon = icon("arrow-down"), color = "red", fill = TRUE)
  })
  output$area_ganada <- renderInfoBox({
    areas <- calculate_areas()
    infoBox("Área con Ganancia", paste(format(areas$positivos, big.mark=","), "km²"), icon = icon("arrow-up"), color = "green", fill = TRUE)
  })
  output$table_uac_complete <- renderDT({
    ...
    req(!is.null(df))
    tryCatch({
      df_display <- data.frame(UAC = gsub("_", " ", df$UAC), Area_Presente = df$Area_Presente_km2, Area_Futura = df$Area_Futura_km2,
                              Perdida = df$Perdida_km2, Ganancia = df$Ganancia_km2, Sin_Cambios = df$Sin_Cambios_km2)
      datatable(df_display, options = list(pageLength = 10, dom = 't', scrollX = TRUE, columnDefs = list(list(className = 'dt-center', targets = 1:5))),
                rownames = FALSE, colnames = c("UAC", "Área Presente (km²)", "Área Futura (km²)", "Pérdida (km²)", "Ganancia (km²)", "Sin Cambios (km²)")) %>%
        formatRound(columns = 2:6, digits = 2) %>%
        formatStyle('Area_Presente', backgroundColor = '#E3F2FD', fontWeight = 'bold') %>%
        formatStyle('Area_Futura', backgroundColor = '#FFF3E0', fontWeight = 'bold') %>%
        formatStyle('Perdida', backgroundColor = '#FFEBEE', color = '#C62828') %>%
        formatStyle('Ganancia', backgroundColor = '#E8F5E9', color = '#2E7D32') %>%
        formatStyle('Sin_Cambios', backgroundColor = '#F5F5F5')
    }, error = function(e) {
  ...
  output$changes_barplot <- renderPlot({
    ...
    par(mar = c(5, 5, 3, 2))
    bp <- barplot(valores, names.arg = c("Cambios\nNegativos", "Sin\nCambios", "Cambios\nPositivos"), col = c("#E74C3C", "#F39C12", "#27AE60"),
                  border = "white", main = "Distribución de Cambios por Categoría", ylab = "Área (km²)", ylim = c(0, max(valores) * 1.2), las = 1)
    text(bp, valores, labels = format(round(valores, 0), big.mark = ","), pos = 3, cex = 0.9, font = 2)
  ...
  output$uac_comparison_plot <- renderPlot({
    ...
    tryCatch({
      ...
      bp1 <- barplot(areas_matrix, beside = TRUE, col = c("#5E35B1", "#FF6F00"), main = "Área Presente vs Futura por UAC",
                     ylab = "Área (km²)", ylim = c(0, max_y1), las = 2, cex.names = 0.9, border = NA)
      legend("top", legend = c("Presente", "Futuro 2050"), fill = c("#5E35B1", "#FF6F00"), bty = "n", horiz = TRUE, xpd = TRUE)
      ...
      bp2 <- barplot(cambios_matrix, beside = TRUE, col = c("#E74C3C", "#95A5A6", "#27AE60"), main = "Cambios por Categoría y UAC",
                     ylab = "Área (km²)", ylim = c(0, max_y2), las = 2, cex.names = 0.9, border = NA)
      legend("top", legend = c("Pérdida", "Sin Cambios", "Ganancia"), fill = c("#E74C3C", "#95A5A6", "#27AE60"), bty = "n", horiz = TRUE, xpd = TRUE)
    }, error = function(e) {
  ...
  output$spider_plot <- renderPlot({
    ...
    max_val <- 100
    min_val <- 0
    spider_data <- rbind(rep(max_val, 12), rep(min_val, 12), datos$Presente, datos$Futuro_2050_SSP126, datos$Futuro_2050_SSP585)
    ...
    radarchart(spider_data, axistype = 1, pcol = c("#2E86C1", "#27AE60", "#E74C3C"),
               pfcol = c(rgb(0.18, 0.53, 0.76, 0.3), rgb(0.15, 0.68, 0.38, 0.3), rgb(0.91, 0.30, 0.24, 0.3)),
               plwd = 3, plty = 1, cglcol = "grey70", cglty = 1, cglwd = 0.8, axislabcol = "grey30", vlcex = 1.4, calcex = 1.1, seg = 5, title = "")
    ...
    legend(x = "topright", legend = c("Presente", "Futuro 2050 SSP 1-2.6", "Futuro 2050 SSP 5-8.5"),
           col = c("#2E86C1", "#27AE60", "#E74C3C"), lty = 1, lwd = 3, bty = "n", cex = 1.2)
    legend(x = "topright", y = NULL, inset = c(0, 0.19), legend = c("Rango:", rango_presente, rango_ssp126, rango_ssp585),
           col = c("black", "#2E86C1", "#27AE60", "#E74C3C"), pch = c(NA, 15, 15, 15), bty = "n", cex = 1.2, pt.cex = 1.8)
    mtext(paste0("Porcentaje del área de estudio (Área total = ", format(round(total_area, 0), big.mark = ","), " km²) | Umbral: ", input$threshold, "%"),
          side = 1, line = 2.5, cex = 1.0, col = "grey20")
  })
  output$spider_data_table <- renderDT({
    datos <- monthly_data()
    req(!is.null(datos))
    datos_display <- data.frame(Mes = datos$Mes, Presente_Porc = datos$Presente, Presente_km2 = datos$Presente_km2,
                                Futuro_SSP126_Porc = datos$Futuro_2050_SSP126, Futuro_SSP126_km2 = datos$Futuro_2050_SSP126_km2,
                                Futuro_SSP585_Porc = datos$Futuro_2050_SSP585, Futuro_SSP585_km2 = datos$Futuro_2050_SSP585_km2)
    colnames(datos_display) <- c("Mes", "Presente (%)", "Presente (km²)", "Futuro SSP 1-2.6 (%)", "Futuro SSP 1-2.6 (km²)",
                                 "Futuro SSP 5-8.5 (%)", "Futuro SSP 5-8.5 (km²)")
    datatable(datos_display, options = list(pageLength = 12, dom = 't', scrollX = TRUE, columnDefs = list(list(className = 'dt-center', targets = 1:6))),
              rownames = FALSE) %>%
      formatRound(columns = 2:7, digits = 2) %>%
      formatStyle(c('Presente (%)', 'Presente (km²)'), backgroundColor = '#E3F2FD', fontWeight = 'bold') %>%
      formatStyle(c('Futuro SSP 1-2.6 (%)', 'Futuro SSP 1-2.6 (km²)'), backgroundColor = '#E8F5E9', fontWeight = 'bold') %>%
      formatStyle(c('Futuro SSP 5-8.5 (%)', 'Futuro SSP 5-8.5 (km²)'), backgroundColor = '#FFEBEE', fontWeight = 'bold')
  })
}

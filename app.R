...
if (needs_download) {
  cat("========================================\n")
  cat("INICIANDO DESCARGA DE DATOS DESDE GITHUB\n")
  cat("========================================\n")
  ...
  for (ext in shp_components) {
    ...
    if (download_with_retry(url, dest, max_attempts = 5, timeout = 300)) {
      ...
      if (size > 0) {
        ...
        cat("FALLO - archivo vacío\n")
        shp_success <- FALSE
      }
    } else {
      cat("FALLO - no se pudo descargar\n")
      shp_success <- FALSE
    }
  }
  
  if (!shp_success) {
    stop("ERROR CRÍTICO: No se pudo descargar el shapefile UACs.")
  }
  ...
  for (sp in especies) {
    ...
    for (mes in meses) {
      ...
      if (file_count %% 30 == 0) {
        cat("  Progreso:", file_count, "/", total_files, "\n")
      }
    }
  }
  ...
  cat("Total archivos procesados:", file_count, "\n")
  cat("Archivos fallidos:", failed_count, "\n")
  cat("========================================\n\n")
} else {
...
ui <- dashboardPage(
  skin = "blue",
  dashboardHeader(title = "Modelos de Idoneidad Ambiental para camarones en el Océano Pacífico colombiano", titleWidth = 750),
  dashboardSidebar(
    width = 300,
    sidebarMenu(
    ...
    tabItems(
      tabItem(tabName = "maps",
              ...
              fluidRow(
                ...
                box(title = "Cambios en Idoneidad Ambiental", status = "info", solidHeader = TRUE, width = 4, height = 600, 
                    leafletOutput("map_changes", height = 520))
              ),
              fluidRow(box(title = "Análisis Cuantitativo de Cambios", status = "info", solidHeader = TRUE, width = 12, 
                           plotOutput("changes_barplot", height = 300)))
      ),
      tabItem(tabName = "uacs",
              fluidRow(box(title = "Mapa con UACs", status = "primary", solidHeader = TRUE, width = 12, height = 600, 
                           leafletOutput("map_uacs", height = 520))),
              fluidRow(box(title = "Análisis Detallado por UAC", status = "info", solidHeader = TRUE, width = 12, 
                           DTOutput("table_uac_complete"))),
              fluidRow(box(title = "Comparación Visual por UAC", status = "primary", solidHeader = TRUE, width = 12, 
                           plotOutput("uac_comparison_plot", height = 400)))
      ),
      tabItem(tabName = "temporal",
              fluidRow(box(title = "Variación Mensual del Área de Distribución", status = "primary", solidHeader = TRUE, 
                           width = 12, plotOutput("spider_plot", height = 650))),
              fluidRow(box(title = "Resumen de Resultados por Especie", status = "success", solidHeader = TRUE, width = 12, 
                           verbatimTextOutput("species_summary"))),
              fluidRow(box(title = "Tabla de Datos Mensuales", status = "info", solidHeader = TRUE, width = 12, 
...
server <- function(input, output, session) {
  ...
  raster_data <- reactiveValues(present_raw = NULL, future_raw = NULL, present_prob = NULL, future_prob = NULL,
                                present_binary = NULL, future_binary = NULL, changes = NULL, uacs = NULL, 
  ...
  calculate_total_study_area <- function(species, base_path) {
    tryCatch({
      ...
      if (!file.exists(pres_file)) return(NULL)
  ...
  calculate_monthly_areas <- function(species, base_path, threshold) {
    ...
    if (is.null(total_area)) return(NULL)
    ...
    resultados <- data.frame(Mes = meses_nombres, Presente = numeric(12), Futuro_2050_SSP126 = numeric(12),
                             ...
                             Futuro_2050_SSP126_km2 = numeric(12), Futuro_2050_SSP585_km2 = numeric(12), 
  ...
  generate_species_summary <- function(datos, species_name) {
    ...
    if (cambio_ssp126_km2 > 0) {
      texto_ssp126 <- paste0("una expansión de ", format(round(cambio_ssp126_km2, 2), big.mark = ","), 
                             " km² (+", round(abs(cambio_ssp126_porc), 2), "% de ganancia)")
    } else {
      texto_ssp126 <- paste0("una contracción de ", format(round(abs(cambio_ssp126_km2), 2), big.mark = ","), 
                             " km² (", round(cambio_ssp126_porc, 2), "% de pérdida)")
    }
    
    if (cambio_ssp585_km2 > 0) {
      texto_ssp585 <- paste0("una expansión significativa a ", format(round(prom_ssp585_km2, 2), big.mark = ","), 
      ...
      texto_ssp585 <- paste0("una reducción drástica a ", format(round(prom_ssp585_km2, 2), big.mark = ","), 
    ...
    parrafo <- paste0("El análisis del área idónea para la distribución potencial de ", species_name, 
                      ...
                      " km² (", round(prom_presente_porc, 2), "% de las Unidades Ambientales Costeras, ", 
                      format(area_uac, big.mark = ","), " km²) en el período presente. ",
                      ...
                      format(round(prom_ssp126_km2, 2), big.mark = ","), " km² (", round(prom_ssp126_porc, 2), 
                      "%), representando ", texto_ssp126, ". ", "En contraste, el escenario pesimista SSP5-8.5 proyecta ", 
                      texto_ssp585, ".")
    return(parrafo)
  }
  ...
  load_raster_data <- function() {
    tryCatch({
      withProgress(message = 'Cargando datos...', value = 0, {
        ...
        if (!file.exists(uac_path)) {
          showNotification("Shapefile UACs no encontrado.", type = "error", duration = NULL)
          return(FALSE)
        }
        ...
        uacs <- tryCatch({
          ...
          showNotification(paste("Error al cargar shapefile:", e$message), type = "error", duration = NULL)
          return(NULL)
        })
        ...
        if (is.null(name_col)) {
          showNotification("No se pudo identificar la columna de nombres.", type = "error", duration = NULL)
          return(FALSE)
        }
  ...
  output$map_present <- renderLeaflet({
    ...
    m <- leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
      ...
      addLegend(pal = pal, values = c(0, 25, 50, 75, 100), title = "Probabilidad (%)", 
                position = "bottomright", opacity = 1) %>% addScaleBar(position = "bottomleft")
  ...
  output$map_changes <- renderLeaflet({
    ...
    m <- leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
      ...
      addLegend(colors = c("#E74C3C", "#F39C12", "#27AE60", "#CCCCCC"), 
                ...
                title = "Categorías", position = "bottomright") %>% addScaleBar(position = "bottomleft")
  ...
  output$map_uacs <- renderLeaflet({
    ...
    tryCatch({
      ...
      m <- m %>% addPolygons(data = uacs, color = "#000000", weight = 2, fillOpacity = 0, label = uac_names, 
                             popup = paste0("<strong>", uac_names, "</strong>"),
      ...
      return(leaflet() %>% setView(lng = -78.5, lat = 3, zoom = 7) %>% addProviderTiles(providers$Esri.OceanBasemap) %>%
               addControl(html = paste0("<div style='background: rgba(255,255,255,0.9); padding: 15px; ",
                                        "border-radius: 5px; border: 2px solid #E74C3C;'>",
                                        "<strong style='color: #E74C3C;'>Error al cargar mapa de UACs</strong><br>",
                                        "Consulte la tabla y gráficos más abajo para el análisis detallado.</div>"), 
                          position = "topright"))
    })
  })
  ...
  calculate_areas <- reactive({
    ...
    tryCatch({
      ...
      list(negativos = round(sum(vals == 1, na.rm = TRUE) * pixel_area, 2), 
  ...
  calculate_uac_areas <- reactive({
    ...
    tryCatch({
      ...
      for (i in 1:nrow(uacs)) {
        ...
        results[[uac_name]] <- list(UAC = uac_name, Area_Presente_km2 = round(area_presente, 2), 
                                    ...
                                    Cambio_Porcentaje = round(ifelse(area_presente > 0, 
                                                                     ((area_futura - area_presente) / area_presente) * 100, 0), 2),
                                    Perdida_km2 = round(negativos, 2), Sin_Cambios_km2 = round(sin_cambios, 2), 
  ...
  output$table_uac_complete <- renderDT({
    ...
    req(!is.null(df))
    tryCatch({
      df_display <- data.frame(UAC = gsub("_", " ", df$UAC), Area_Presente = df$Area_Presente_km2, 
                               Area_Futura = df$Area_Futura_km2, Perdida = df$Perdida_km2, 
                               Ganancia = df$Ganancia_km2, Sin_Cambios = df$Sin_Cambios_km2)
      datatable(df_display, options = list(pageLength = 10, dom = 't', scrollX = TRUE, 
                                           columnDefs = list(list(className = 'dt-center', targets = 1:5))),
                rownames = FALSE, colnames = c("UAC", "Área Presente (km²)", "Área Futura (km²)", 
                                               "Pérdida (km²)", "Ganancia (km²)", "Sin Cambios (km²)")) %>%
  ...
  output$changes_barplot <- renderPlot({
    ...
    bp <- barplot(valores, names.arg = c("Cambios\nNegativos", "Sin\nCambios", "Cambios\nPositivos"), 
                  ...
                  main = "Distribución de Cambios por Categoría", ylab = "Área (km²)", 
                  ylim = c(0, max(valores) * 1.2), las = 1)
  ...
  output$uac_comparison_plot <- renderPlot({
    ...
    tryCatch({
      ...
      bp1 <- barplot(areas_matrix, beside = TRUE, col = c("#5E35B1", "#FF6F00"), 
                     main = "Área Presente vs Futura por UAC", ylab = "Área (km²)", ylim = c(0, max_y1), 
                     las = 2, cex.names = 0.9, border = NA)
      legend("top", legend = c("Presente", "Futuro 2050"), fill = c("#5E35B1", "#FF6F00"), 
      ...
      bp2 <- barplot(cambios_matrix, beside = TRUE, col = c("#E74C3C", "#95A5A6", "#27AE60"), 
                     main = "Cambios por Categoría y UAC", ylab = "Área (km²)", ylim = c(0, max_y2), 
                     las = 2, cex.names = 0.9, border = NA)
      legend("top", legend = c("Pérdida", "Sin Cambios", "Ganancia"), 
             fill = c("#E74C3C", "#95A5A6", "#27AE60"), bty = "n", horiz = TRUE, xpd = TRUE)
    }, error = function(e) {
      plot(1, type = "n", axes = FALSE, xlab = "", ylab = "")
  ...
  output$spider_plot <- renderPlot({
    ...
    max_val <- 100
    min_val <- 0
    spider_data <- rbind(rep(max_val, 12), rep(min_val, 12), datos$Presente, 
                         datos$Futuro_2050_SSP126, datos$Futuro_2050_SSP585)
    ...
    radarchart(spider_data, axistype = 1, pcol = c("#2E86C1", "#27AE60", "#E74C3C"),
               ...
               plwd = 3, plty = 1, cglcol = "grey70", cglty = 1, cglwd = 0.8, axislabcol = "grey30", 
               vlcex = 1.4, calcex = 1.1, seg = 5, title = "")
    ...
    legend(x = "topright", y = NULL, inset = c(0, 0.19), 
           legend = c("Rango:", rango_presente, rango_ssp126, rango_ssp585),
           col = c("black", "#2E86C1", "#27AE60", "#E74C3C"), pch = c(NA, 15, 15, 15), 
           bty = "n", cex = 1.2, pt.cex = 1.8)
    mtext(paste0("Porcentaje del área de estudio (Área total = ", format(round(total_area, 0), big.mark = ","), 
                 " km²) | Umbral: ", input$threshold, "%"), side = 1, line = 2.5, cex = 1.0, col = "grey20")
  })
  ...
  output$spider_data_table <- renderDT({
    ...
    colnames(datos_display) <- c("Mes", "Presente (%)", "Presente (km²)", "Futuro SSP 1-2.6 (%)", 
                                 "Futuro SSP 1-2.6 (km²)", "Futuro SSP 5-8.5 (%)", "Futuro SSP 5-8.5 (km²)")
    datatable(datos_display, options = list(pageLength = 12, dom = 't', scrollX = TRUE, 
